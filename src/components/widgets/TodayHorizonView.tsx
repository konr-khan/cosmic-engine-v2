  ...
  return (
    <div className="flex flex-col h-full w-full justify-between select-none relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1 items-stretch">
        ...
        <div className="bg-slate-900/60 rounded-xl p-3.5 border border-slate-800/80 flex flex-col justify-between shadow-inner backdrop-blur-sm">
          {/* Header */}
          <div className="w-full flex justify-between items-center mb-1 px-1 font-mono">
            <div className="text-xs text-slate-300 font-bold uppercase tracking-wider flex items-center gap-1.5">
          ...
          <div className="relative w-full py-1 flex items-center justify-center">
            <svg viewBox="0 0 200 88" className="w-full max-h-[105px] overflow-visible" preserveAspectRatio="xMidYMid meet">
              ...
              <text x="18" y={elCy + 10} textAnchor="end" className="text-[8px] font-mono fill-slate-400 font-bold">0°</text>
              <text x="182" y={elCy + 10} textAnchor="start" className="text-[8px] font-mono fill-slate-400 font-bold">0°</text>
              ...
              <text x={elCx} y={elCy - elR - 5} textAnchor="middle" className="text-[8px] font-mono fill-slate-400 font-bold">+90°</text>
          ...
          <div className="text-center my-1 bg-slate-950/95 px-3 py-1 rounded-lg border border-slate-800 shadow-md">
            <div className={`text-sm font-mono font-bold ${currentSunElevation >= 0 ? 'text-amber-400' : 'text-slate-400'}`}>
          ...
          <div className="grid grid-cols-3 gap-2 w-full bg-slate-950/80 p-2 rounded-xl border border-slate-800/80 text-xs font-mono mt-1">
        ...
        <div className="bg-slate-900/60 rounded-xl p-3.5 border border-slate-800/80 flex flex-col justify-between shadow-inner backdrop-blur-sm relative">
          {/* Header */}
          <div className="w-full flex justify-between items-center mb-1 px-1 font-mono">
            <div className="text-xs text-slate-300 font-bold uppercase tracking-wider flex items-center gap-1.5">
              <Moon className="w-4 h-4 text-cyan-400" /> Moon Elevation Arc
            </div>
            <div className="text-xs font-bold text-cyan-400">
              Transit Peak: <strong className="text-white">{transitPeakElevation.toFixed(1)}°</strong>
            </div>
          </div>
              ...
              {currentMoonElevation > -18 && (
                <g>
                  <line
                    ...
                    x2={moonX}
                    y2={moonY}
                    stroke={currentMoonElevation >= 0 ? '#38bdf8' : '#64748b'}
                    strokeWidth="1.2"
                    strokeDasharray="2 2"
                    opacity="0.85"
                  />
                  <circle
                    ...
                    cy={moonY}
                    r="5.5"
                    fill={currentMoonElevation >= 0 ? '#38bdf8' : '#475569'}
                    stroke="#ffffff"
                    strokeWidth="1.5"
                    className="drop-shadow"
                  />
          ...
          <div className="text-center my-1 bg-slate-950/95 px-3 py-1 rounded-lg border border-slate-800 shadow-md">
            <div className={`text-sm font-mono font-bold ${currentMoonElevation >= 0 ? 'text-cyan-400' : 'text-slate-400'}`}>
          ...
          {isHoveringMoonMetrics && (
            <div className="absolute bottom-20 right-4 z-30 bg-slate-900/95 backdrop-blur-md border border-slate-700 p-3 rounded-xl max-w-xs shadow-2xl font-mono space-y-1 pointer-events-none animate-in fade-in zoom-in-95 duration-150">
              <div className="text-xs font-bold text-cyan-400 flex items-center justify-between">
                ...
                <span className="text-slate-400 text-[10px]">{illPercent}% Illum</span>
              </div>
              <div className="text-[10px] text-slate-300">
                Distance: <strong className="text-white">{(distanceKm || 384400).toLocaleString()} km</strong>{' '}
                <span className="text-slate-400">({(distanceEarthRadii || ((distanceKm || 384400) / 6371)).toFixed(1)} R_E)</span>
              </div>
              <div className="text-[10px] text-slate-300">
                Parallactic Angle: <strong className="text-indigo-300">{parallacticAngle.toFixed(1)}°</strong>
              </div>
          ...
          <div 
            className="bg-slate-950/80 p-2 rounded-xl border border-slate-800/80 flex items-center justify-between gap-3 mt-1 cursor-pointer transition-colors hover:border-slate-700"
          ...
          <div className="grid grid-cols-3 gap-2 w-full bg-slate-950/80 p-2 rounded-xl border border-slate-800/80 text-xs font-mono mt-1">
            ...
            <div
              ...
              className="text-center bg-indigo-950/80 hover:bg-indigo-900/90 transition-all cursor-pointer p-1.5 rounded-lg border border-indigo-500/50 text-indigo-300 shadow-sm"
              title="Click to jump clock to Lunar Transit"
            >
              <span className="text-[9px] text-indigo-400 block uppercase font-bold flex items-center justify-center gap-0.5">
                <Compass className="w-2.5 h-2.5" /> Lunar Transit
              </span>
              <span className="text-indigo-200 font-bold text-xs">{formatTime(transit).substring(0, 5)} UTC</span>
            </div>
            <div className="text-center bg-slate-900/60 p-1.5 rounded-lg border border-slate-800/60">
              <span className="text-[9px] text-slate-400 block uppercase font-bold">Declination (δ)</span>
              <span className={`text-xs font-bold ${moonDeclination >= 0 ? 'text-indigo-400' : 'text-rose-400'}`}>
